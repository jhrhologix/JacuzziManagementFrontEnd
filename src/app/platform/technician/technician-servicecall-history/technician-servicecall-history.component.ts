...
convertToken(token: any) {
  ...
  const serviceCallIdString = this.adminToken[0]; // Adjust index if needed
  const parsedserviceCallId = JSON.parse(serviceCallIdString);
  this.serviceCallId = parsedserviceCallId.serviceCallId;
